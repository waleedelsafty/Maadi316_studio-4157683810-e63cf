...
export default function LevelPage() {
    ...
    const [unitOwnerId, setUnitOwnerId] = useState<string | null>(null);
    ...
    const handleAddUnit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!unitNumber.trim() || unitSqm === '' || unitMaintenance === '' || !unitOwnerId || !unitTypeId || !firestore || !buildingId) {
        ...
        const newUnitData: Omit<Unit, 'id' | 'createdAt'> & { createdAt: any } = {
            ...
            unitTypeId: unitTypeId, createdAt: serverTimestamp(),
        };

        const unitsCollectionRef = collection(firestore, 'buildings', buildingId, 'units');
        addDoc(unitsCollectionRef, newUnitData)
            .then(() => {
                setUnitNumber(''); setUnitSqm(''); setUnitMaintenance(''); setUnitOwnerId(null); setUnitTypeId('');
    ...
    const availableUnitTypeIds = building?.enabledUnitTypeIds || [];
                        ...
                        <form onSubmit={handleAddUnit} className="space-y-4 p-4 border rounded-lg bg-muted/50">
                            <h3 className="font-medium">Add a New Unit</h3>
                             <div className="grid sm:grid-cols-2 gap-4">
                                <Input placeholder="Unit Number (e.g., '101', 'G-02')" value={unitNumber} onChange={(e) => setUnitNumber(e.target.value)} required />
                                 <Select onValueChange={setUnitTypeId} value={unitTypeId}>
                                    <SelectTrigger><SelectValue placeholder="Select Unit Type" /></SelectTrigger>
                                    <SelectContent>
                                        {availableUnitTypeIds.length > 0 ? (
                                            availableUnitTypeIds.map(typeId => (<SelectItem key={typeId} value={typeId}>{unitTypesMap.get(typeId) || typeId}</SelectItem>))
                                        ) : (
                            ...
                            <div>
                                <OwnerCombobox
                                    ...
                                    owners={owners || []}
                                    value={unitOwnerId}
                                    onChange={setUnitOwnerId}
                                />
                            </div>
                            <div className="flex gap-2">
                                <Button type="submit">Save Unit</Button>
                                <Button variant="outline" type="button" onClick={() => setIsAddingUnit(false)}>Cancel</Button>
                            </div>
                        </form>
                    )}
                                    ...
                                    <TableBody>
                                        {units.map((unit) => (
                                            <TableRow key={unit.id}>
                                                <TableCell className="font-semibold py-1 px-2">{unit.unitNumber}</TableCell>
                                                <TableCell className="py-1 px-2">{unitTypesMap.get(unit.unitTypeId) || unit.unitTypeId}</TableCell>
                                                <TableCell className="py-1 px-2">{unit.sqm}</TableCell>
                                                <TableCell className="py-1 px-2">{ownersMap.get(unit.ownerId) || 'N/A'}</TableCell>
                                                <TableCell className="py-1 px-2">{unit.quarterlyMaintenanceFees.toLocaleString(undefined, { style: 'currency', currency: 'USD' })}</TableCell>
