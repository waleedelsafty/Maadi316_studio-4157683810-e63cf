        ...
        <TableRow>
            ...
            <TableCell className="text-right">
                <div className="flex gap-2 justify-end">
                    <Button variant="outline" size="sm" asChild><Link href={`/building/${buildingId}/level/${level.id}`}>Edit</Link></Button>
                    <AlertDialog>
...
export default function BuildingStructurePage() {
    ...
    const availableLevelTypes = useMemo(() => {
        ...
        if (building.hasBasement) {
            if (levels.filter(l => l.type === 'Basement').length >= (building.basementCount || 0)) {
        ...
        if (building.hasMezzanine) {
            if (levels.filter(l => l.type === 'Mezzanine').length >= (building.mezzanineCount || 0)) {
    ...
    const sortedLevels = useMemo(() => {
        ...
        return [...levels].sort((a, b) => {
            ...
            switch (sortKey) {
                ...
                case 'type':
                default:
                    const typeA = levelTypeOrder[a.type]; const typeB = levelTypeOrder[b.type];
                    if (typeA !== typeB) return (typeA - typeB) * dir;
                    if (a.type === 'Typical Floor') return ((a.floorNumber || 0) - (b.floorNumber || 0)) * dir;
                    if (a.type === 'Basement') return aName.localeCompare(bName) * (dir * -1);
