...
import type { UserProfile, Unit, Owner, Level, Building, AppSettings, GlobalUnitType } from '@/types';
...
export default function UnitInspectorPage2() {
    ...
    const selectedUnitRef = React.useMemo(() => {
    ...
    const { data: selectedUnit } = useDoc<Unit>(selectedUnitRef);
    ...
    const unitTypesMap = React.useMemo(() => new Map((globalUnitTypes || []).map(t => [t.id, t.name])), [globalUnitTypes]);
    ...
    React.useEffect(() => {
        if (selectedUnit) {
            ...
            reset({ unitNumber: '', sqm: 0, quarterlyMaintenanceFees: 0, unitTypeId: '', levelId: '', ownerId: '' });
        }
    }, [selectedUnit, reset]);
    ...
    const onSubmit = async (data: z.infer<typeof formSchema>) => {
        ...
        updateDoc(selectedUnitRef, submissionData)
            .then(() => toast({ title: 'Unit Updated', description: `The details for Unit ${data.unitNumber} have been saved.` }))
            .catch(() => errorEmitter.emit('permission-error', new FirestorePermissionError({ path: selectedUnitRef.path, operation: 'update', requestResourceData: submissionData })));
    };
    ...
    const isLoading = userProfile === undefined || (defaultBuildingId && (allUnits === null || owners === null || levels === null || building === undefined || globalUnitTypes === undefined));
                ...
                <CardContent>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div><Label htmlFor="unitTypeId">Unit Type</Label><Controller name="unitTypeId" control={control} render={({ field }) => (<Select onValueChange={field.onChange} value={field.value}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{availableUnitTypes.map(type => <SelectItem key={type.id} value={type.id}>{type.name}</SelectItem>)}</SelectContent></Select>)} />{errors.unitTypeId && <p className="text-destructive text-xs mt-1">{errors.unitTypeId.message}</p>}</div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
