...
export function LevelFormSheet({ level, buildingId, isOpen, onOpenChange, existingLevels }: LevelFormSheetProps) {
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    ...
    if (data.type === 'Typical Floor') {
      ...
      if (existingLevels.some(l => l.id !== level.id && l.type === 'Typical Floor' && l.floorNumber === data.floorNumber)) {
        toast({
          variant: 'destructive',
    ...
    if (uniqueLevelTypes.includes(data.type)) {
      if (existingLevels.some(l => l.id !== level.id && l.type === data.type)) {
         toast({
          variant: 'destructive',
          title: 'Duplicate Level Type',
          description: `A level of type "${data.type}" already exists in this building.`,
        });
        return;
      }
    }
    ...
    const submissionData = {
        name: data.name,
        type: data.type,
        floorNumber: data.type === 'Typical Floor' ? data.floorNumber : null,
    };
  ...
  const availableEditLevelTypes = React.useMemo(() => {
     ...
     uniqueLevelTypes.forEach(uniqueType => {
         if (otherLevels.some(l => l.type === uniqueType)) {
